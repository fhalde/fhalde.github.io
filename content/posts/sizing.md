...
Here \(K_{tok}\) is measured in **bytes per token**. The factor of 2 is for keys and values. This formula is grouped-query-attention aware because it uses KV heads, not total attention heads.
